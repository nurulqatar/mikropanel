...
class ClientIdentityOcrService
{
    ...
    private function ocrImage(
        ...
        if ($looksLikeQatarId) {
            ...
            /*
             * Qatar ID back-side values are much
             * smaller than the labels. The normal
             * whole-card OCR can therefore read
             * "Residency Type" / "Employer" but
             * damage the actual Arabic value.
             *
             * Only for a detected back side, enlarge
             * the field region and run an additional
             * Arabic-priority bilingual OCR pass.
             */
            if (
                $this->looksLikeQatarIdBackOcrText(
                    $combined
                )
            ) {
                $backVariants =
                    $this->prepareQatarBackOcrVariants(
                        $enhanced
                    );
    ...
    private function looksLikeQatarIdBackOcrText(
        string $text
    ): bool {
        $passportNumber =
            (bool) preg_match(
    ...
    private function prepareQatarBackOcrVariants(
        string $enhanced
    ): array {
        if (
            !is_executable(
        ...
        /*
         * Qatar residency-card back:
         * passport/residency/employer fields occupy
         * the upper/central majority of the card.
         *
         * Exclude a small top border and most of the
         * lower signature/authority decoration so
         * Tesseract receives larger useful text.
         */
        $cropTop =
            max(
                0,
        ...
        if (
            ...
            /*
             * Second high-contrast copy. Some cards
             * have security backgrounds that confuse
             * Arabic character strokes.
             */
            $bw =
                $directory
                . '/ocr-qid-back-bw.png';
